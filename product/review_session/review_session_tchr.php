...
.table_btn{width:37px; height:33px; cursor:pointer}
@media (max-width: 700px) {#create_new{font-size:15px; width:150px;}}
</style>
</head>
<body>
...
<?php
// see if poll records exist in database
$result = mysqli_query($con,"select * from review_session where course_id = $course_id group by title") or die("Error: " . mysqli_error($con));
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = $result->fetch_assoc()) {
    	 $title = $row["title"];
         echo "<tr><td data-th='Title'>".ucwords($title).
			"</td><td data-th='Status'>".strtoupper($row["status"])."</td>";
		 echo "<td data-th='Close Poll?'>";
		 // add close poll subfeature
         echo "<form action='#' name='close_poll' method = 'post'>";
         echo "<button name='close' class='table_btn' value='$title' onClick='confirm_message();'><i class='fas fa-times-circle fa-lg'></i></button>";
         echo "</form>";
         echo "</td>";
         
         // add see poll result subfeature
		 echo "<td data-th='Result'>";
         echo "<form action='poll_result.php' method = 'post'>";
         echo "<button name='result' class='table_btn' value='$title'><i class='fas fa-poll fa-lg'></i></button>";
         echo "</form>";
         echo "</td>";
    }
    echo "</tbody>";
...
<!-- create new poll subfeature -->
<form action="create_poll.php" method="post">
<button id='create_new' class="btn" type="submit" style="margin-top: 30px;">Create New Poll</button>
</form>
...
<?php
  ...
  $poll_title = mysqli_real_escape_string($con,$_POST['close']);
  if (mysqli_query($con, "update review_session set status = 'closed' where title='$poll_title'") or die("Error: " . mysqli_error($con))){
  echo 
    	"<script>
    	alert('Poll Closed');
    	setTimeout(function(){window.location.href='review_session_tchr.php';},333);
    	</script> ";
  }
  }
?>
...
<script>
// add confirm message to close poll subfeature
function confirm_message(){
var answer = confirm('Are you sure you want to close this poll?');
if(answer){
close_poll.submit();
}
else{
