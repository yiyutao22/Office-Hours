...
<div class='container'>
<?php
$result = mysqli_query($con,"select * from forum_topic join forum on forum.topic_id=forum_topic.topic_id where course_id = $course_id and post_id IN ( SELECT MAX(post_id) FROM forum GROUP BY topic_id) GROUP BY forum.topic_id order by date DESC") or die("Error: " . mysqli_error($con));
if (mysqli_num_rows($result) > 0) {
	...
	echo "<th>Topic</th>";
	echo "<th>Reply</th>";
	echo "<th># of Replies</th>";
	echo "<th>Last Responded At</th>";
	echo "</tr>";
	echo "</thead>";
    ...
    while($row = $result->fetch_assoc()) {
		 ...
		 echo "<tr>";
		 echo "<td data-th='Topic'>$topic</td>";		
		 echo "<td data-th='Reply'>";
         echo "<form action='forum.php' method = 'get'>";
         echo "<button name='reply' class='table_btn' value='$topic'><i class='fas fa-comment-dots fa-lg'></i></button>";
         echo "</form></td>";
         echo "<td data-th='# of Replies'>";
         $num = mysqli_fetch_array(mysqli_query($con,"select count(*) from forum join forum_topic on forum_topic.topic_id = forum.topic_id where topic_name = '$topic'"))[0];
         echo $num;
         echo "</td>";
         echo "<td data-th='Last Responded At'>";
...
<button class="btn" type="submit" style="margin-top: 30px;">Create New Topic</button>
</form>
