...
if (isset($_POST['submit'])) {
    ...
    $sql="INSERT INTO forum_topic(topic_name) VALUES('$topic')";
    $con->query($sql);
    $topic_id = mysqli_fetch_array(mysqli_query($con,"select topic_id from forum_topic where topic_name='$topic'"))[0];
    $message_sql="INSERT INTO forum(user_id, course_id, message, date, topic_id) VALUES('$uid', '$course_id','$message','$date','$topic_id')";
    if ($con->query($message_sql)){$good=true;};
}
if ($good==true){echo 
    	"<script>
    	alert('New topic has been created successfully!');
    	setTimeout(function(){window.location.href='forum_topic.php';},777);
    	</script> ";}
...
.feature-content input[type="submit"]{width:150px; text-align:left; padding: 12px 19px 12px 19px;}
}
@media (max-width: 825px) {
.feature-content input[type="submit"]{width:140px; text-align:left; padding: 12px 19px 12px 19px;}
}
...
<center><h1>Create A New Discussion Topic for <?php echo $class; ?></h1></center>

<fieldset>
<legend>Topic</legend>
<input type="text" style="font-size:17px;" name="topic" required>
</fieldset>
...
<textarea name="message" id="message" style="font-size:17px;" cols="40" rows="7" maxlength="250" spellcheck="true"></textarea>
  ...
  <input type="submit" name="submit" id="submit" value="Create Topic" />
</div>
</form>
</div>
</div> 
